...
def suitify(parent):
    ...
    suite = Node(syms.suite, [Newline(), Leaf(
        token.INDENT, indentation(node) + indentation_step(node))])
...
def import_binding_scope(node):
    ...
    # Recursively yield nodes following imports inside of a
    # if/while/for/try/with statement
...
def future_import(feature, node):
    ...
    import_ = FromImport(
        u"__future__", [Leaf(token.NAME, feature, prefix=u" ")])

    children = [import_, Newline()]
    root.insert_child(
        insert_pos, Node(syms.simple_stmt, children, prefix=prefix))
