...
def all_patterns(name):
    ...
    if u'.' in name:
        ...
        i_from_submod = from_import_submod_match % (
            simple_name, simple_attr, simple_attr, simple_attr, simple_attr)
...
class FixImports(fixer_base.BaseFix):
    ...
    def fix_simple_name(self, node, mapping=MAPPING):
        ...
        node.replace(
            Leaf(token.NAME, unicode(replacement), prefix=node.prefix))
    ...
    def fix_submod_import(self, imported, name, node):
        ...
        for attr in imported:
            ...
            if dotted in MAPPING:
                ...
                if u'.' not in to_repl:
                    ...
                    _import = NameImport(
                        Name(to_repl, prefix=u" "), attr.value)
        ...
        if missed:
            self.warning(
                node, u"Imported names not known to 3to2 to be part of the package %s.  Leaving those alone... high probability that this code will be incorrect." % (name))
            children = [Name(u"from"), Name(name, prefix=u" "), Name(
                u"import", prefix=u" "), Node(syms.import_as_names, missed)]
